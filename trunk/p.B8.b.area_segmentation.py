#!/usr/bin/env python
#Create new subset of polygon with a Convexity and Area Threshold
import grass.script as grass
################################################
##### Sript area_segmentation.py ###############
...
convex_thresh=raw_input("Please enter the threshold convexity index : ")
area_thresh=raw_input("Please enter the threshold area : ")
...
#for i in []:
    ...
    #transform str to float cat values to get diferent area values
    category=grass.read_command("v.db.select",map='out_poly_1',col='cat',flags='c',fs=' ')
    category2=category.rsplit()
    k=0
    list_cat=[]
    while k<len(category2):
        n=float(category2[k])
        list_cat.append(n)
        k+=1
    #find cat min
    cat_sorted=sorted(list_cat)
    min_cat=cat_sorted[0]
    update_cat="cat-"+str(min_cat)
    grass.run_command("v.db.addcol",map='out_poly_1',col='cat_new int')
    grass.run_command("v.db.update",map='out_poly_1',col='cat_new',value=update_cat)
    grass.run_command("v.db.update",map='out_poly_1',col='area',value='area+cat_new/13000')
    ...
    while area_sorted.count(0)!=len(area_sorted):
        ...
        while len(list_neig)!=0:
            ...
            #extract the new category and area values
            q=0
            list_flo_neig=[] 
            while q<len(list_neig):
                list_flo_neig.append(list_neig[q])
                list_flo_neig.append(list_flo[list_flo.index(list_neig[q])+1])
                q+=1
            cat_max2=int(list_flo_neig[list_flo_neig.index(max2)-1])
            ...
            accum_area=grass.read_command("v.db.select",map='new_set_disolved',col='area',flags='c')
            accum_area1=accum_area.rsplit()
            accum_area2=accum_area1[0]
            accum_area3=float(accum_area2)           
            if convexity>float(convex_thresh) and float(area_thresh)>accum_area3:
...
grass.run_command("v.db.update",map='convexity_dissolved',col='cat_',value='cat/10000')
...
if int(dissolve)==1:
    ...
    grass.run_command("v.db.update",map='convexity_dissolved_2',col='cat_',value='cat/10000')
    #some mistakes with transform double to integer
    grass.run_command("v.db.addcol",map='convexity_dissolved_2',col='cat_2 integer')
    grass.run_command("v.db.update",map='convexity_dissolved_2',col='cat_2',value='cat_+2')
    grass.run_command("v.db.update",map='convexity_dissolved_2',col='cat_',value='cat_2-2')
    grass.run_command("v.db.addcol",map='convexity_dissolved_2',col='cat_3 integer')
    grass.run_command("v.db.update",map='convexity_dissolved_2',col='cat_3',value='cat_2-2')
    grass.run_command("v.db.dropcol",map='convexity_dissolved_2',col='cat_')
...
#while n < 6:
    ...
    for i in new_cats.splitlines():
        ...
        id1=id.rsplit()
        id2=id1[0]
        grass.run_command("v.db.update",map='convexity_dissolved_2',col=column,value=id2,where="cat_3=%s"%aux)
    n+=1
grass.run_command("v.db.dropcol",map='convexity_dissolved_2',col='cat_')
grass.run_command("v.db.dropcol",map='convexity_dissolved_2',col='cat_2')
grass.run_command("v.db.dropcol",map='convexity_dissolved_2',col='cat_3')
